...
public class GuarantorController {
    ...
    public ResponseEntity<Map<String, Object>> submitGuarantorForm(
            ...
            @RequestParam("guarantorPhoneNumber") String guarantorPhoneNumber,
            @RequestParam("guarantorNin") String guarantorNin,
            @RequestParam(value = "selfieImage", required = false) MultipartFile selfieImage) {

        GuarantorRequest request = new GuarantorRequest();
        request.setGuarantorBvn(guarantorBvn);
        request.setGuarantorPhoneNumber(guarantorPhoneNumber);
        request.setGuarantorNin(guarantorNin);

        Map<String, Object> response = guarantorService.submitGuarantorForm(token, request, selfieImage);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<String> confirmGuarantorViaEmail(@PathVariable Long guarantorId) {
        ...
        String phoneHtml = (success && !phone.isEmpty())
                ? "<p class='phone'>OTP has been sent to: <strong>" + phone + "</strong></p>"
                : "";
