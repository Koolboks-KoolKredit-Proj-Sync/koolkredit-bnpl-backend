 ...
 *   2. processReviewDecision()  — applies overrides, sends OTP (APPROVE) or rejection email (REJECT)
 *   3. generateAndSendReviewEmail() — called after submission; generates token + mails admin
 */
@Service
public class AdminCreditReviewService {
    ...
    @Value("${app.frontend.base-url:https://koolkredit-bnpl-frontend.vercel.app}")
    ...
    public Map<String, Object> getReviewData(String token) {
        ...
        // Parse stored credit report JSON
    ...
    public boolean processReviewDecision(String token, AdminReviewDecisionRequest req) {
        ...
        // Apply admin overrides to the entity
        ...
        if ("APPROVE".equalsIgnoreCase(req.getDecision())) {
            // Send OTP now that admin has approved
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 3. Generate review token + send admin email
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public String generateAndSendReviewEmail(AgentFollowUp followUp) {
        ...
        String subject    = classIcon + " New Credit Application — Admin Review Required | BVN: " + safe(followUp.getBvn());

        String htmlBody = buildReviewEmailHtml(followUp, classLabel, classIcon, reviewUrl);

        try {
            // Uses the existing sendEmail — Brevo accepts HTML in the body field
            brevoEmailService.sendEmail(adminEmail, "Admin", subject, htmlBody);
            log.info("Admin review email sent for BVN {} — token: {}", followUp.getBvn(), token);
        } catch (Exception e) {
            log.error("Failed to send admin review email: {}", e.getMessage(), e);
        }
    ...
    private void sendRejectionEmailToCustomer(AgentFollowUp followUp, String reason) {
        ...
        String rejectionUrl = String.format("%s/application-rejected/%s?reason=%s",
                frontendBaseUrl,
        ...
        String body =
                ...
                "Reason: " + (reason != null ? reason : "Please contact support for more information.") + "\n\n" +
                "You may view full details here:\n" + rejectionUrl + "\n\n" +
                "For questions, please contact support@koolboks.com\n\n" +
                "Warm regards,\nThe Koolboks Credit Team";

        try {
            brevoEmailService.sendEmail(followUp.getCustomerEmail(), "Valued Customer",
                    "Your Koolboks Application — Update", body);
    ...
    private String buildReviewEmailHtml(AgentFollowUp followUp,
                                         String classLabel,
                                         String classIcon,
                                         String reviewUrl) {
        ...
        double dti = followUp.getDti() != null ? followUp.getDti() : 0.0;
        ...
        return "<!DOCTYPE html>" +
               ...
               ".badge { display:inline-block; padding:6px 16px; border-radius:999px; font-size:13px; font-weight:700; color:#fff; background:" + classColour + "; margin:12px 0; }" +
               ".row { display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid #1a1a1a; font-size:13px; }" +
               ".lbl { color:#6b7280; } .val { color:#e5e7eb; font-weight:600; }" +
               ".btn-wrap { text-align:center; margin:28px 0 8px; }" +
               ".btn { display:inline-block; padding:14px 40px; background:#000; border:2px solid #f7623b; color:#f7623b; font-size:15px; font-weight:800; border-radius:10px; text-decoration:none; }" +
               ".footer { padding:16px 32px; border-top:1px solid #111; text-align:center; font-size:11px; color:#4b5563; }" +
               "</style></head><body>" +
               "<div class=\"wrap\">" +
               "<div class=\"header\"><h1>Credit Application — Review Required</h1><p>A new application is awaiting your decision</p></div>" +
               "<div class=\"body\">" +
               "<p style=\"color:#9ca3af;font-size:13px;margin-bottom:16px;\">Hello Admin, a new credit application requires your review.</p>" +
               "<div class=\"row\"><span class=\"lbl\">BVN</span><span class=\"val\" style=\"font-family:monospace\">" + safe(followUp.getBvn()) + "</span></div>" +
               "<div class=\"row\"><span class=\"lbl\">Mobile</span><span class=\"val\">" + safe(followUp.getMobileNumber()) + "</span></div>" +
               "<div class=\"row\"><span class=\"lbl\">Plan</span><span class=\"val\">" + safe(followUp.getPlan()) + "</span></div>" +
               "<div class=\"row\"><span class=\"lbl\">Instalment</span><span class=\"val\">" + safe(followUp.getInstallmentOption()) + "</span></div>" +
               "<div class=\"row\"><span class=\"lbl\">Usage</span><span class=\"val\">" + safe(followUp.getUsageType()) + "</span></div>" +
               "<div class=\"row\"><span class=\"lbl\">DTI</span><span class=\"val\">" + String.format("%.2f%%", dti) + "</span></div>" +
               "<div style=\"text-align:center;margin-top:12px;\">" +
               "<p style=\"font-size:12px;color:#6b7280;margin-bottom:4px;\">System Classification</p>" +
               "<span class=\"badge\">" + classIcon + " " + classLabel + "</span>" +
               "</div>" +
               "<div class=\"btn-wrap\"><a href=\"" + reviewUrl + "\" class=\"btn\">Review Application</a></div>" +
               "<p style=\"text-align:center;font-size:12px;color:#4b5563;\">Click the button above to open the full credit report and make your decision.</p>" +
               "</div>" +
               "<div class=\"footer\">Koolboks Credit System &middot; Automated Notification &middot; Confidential</div>" +
               "</div></body></html>";
    }
