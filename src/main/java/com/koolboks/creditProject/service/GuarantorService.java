...
public class GuarantorService {
    ...
    // ─────────────────────────────────────────────────────────────
    // STEP 1: Customer requests guarantor
    // ─────────────────────────────────────────────────────────────
    ...
    public Map<String, Object> requestGuarantor(String customerBvn,
        ...
        try {
            Optional<Guarantor> existing = guarantorRepository
                    .findTopByCustomerBvnAndActiveTrueOrderByCreatedAtDesc(customerBvn);

            if (existing.isPresent()) {
                Guarantor existingGuarantor = existing.get();

                // Always update product fields on existing records
    ...
    // ─────────────────────────────────────────────────────────────
    // STEP 2: Get guarantor data by token
    // ─────────────────────────────────────────────────────────────
    ...
    public Map<String, Object> submitGuarantorForm(String token, GuarantorRequest request, MultipartFile selfieImage) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            if (selfieImage != null && !selfieImage.isEmpty()) {
                try {
                    String uploadDir = System.getProperty("java.io.tmpdir") + "/guarantor-selfies/";
                    java.io.File dir = new java.io.File(uploadDir);
                    if (!dir.exists()) dir.mkdirs();
                    String fileName = "selfie_" + guarantor.getId() + "_" + System.currentTimeMillis() + ".jpg";
                    String filePath = uploadDir + fileName;
                    selfieImage.transferTo(new java.io.File(filePath));
                    guarantor.setSelfieImagePath(filePath);
                    log.info("Selfie saved for guarantor ID: {} at path: {}", guarantor.getId(), filePath);
                } catch (Exception e) {
                    log.warn("Could not save selfie image, continuing without it: {}", e.getMessage());
                }
            }
            ...
            log.info("Guarantor form submitted for customer BVN: {}", guarantor.getCustomerBvn());
    ...
    public Map<String, Object> confirmGuarantor(Long guarantorId) {
        ...
        try {
            ...
            log.info("Guarantor confirmed and OTP saved for ID: {}", guarantorId);
    ...
    public Map<String, Object> verifyGuarantorOtp(String token, String otp) {
        ...
        try {
            ...
            if (otp.equals(guarantor.getOtpCode())) {
                ...
                log.info("OTP verified for guarantor ID: {}", guarantor.getId());

                try {
                    Map<String, Object> offerLetterResult = offerLetterService
                            .generateAndSendOfferLetter(guarantor.getId());

                    if (Boolean.TRUE.equals(offerLetterResult.get("success"))) {
                        log.info("Offer letter sent for guarantor ID: {}", guarantor.getId());
                        ...
                        log.error("Offer letter failed: {}", offerLetterResult.get("message"));
                    ...
                    // Don't fail OTP verification if offer letter fails
                }
    ...
    public Map<String, Object> getGuarantorStatus(String customerBvn) {
        ...
        try {
            Optional<Guarantor> guarantorOpt = guarantorRepository
                    .findTopByCustomerBvnAndActiveTrueOrderByCreatedAtDesc(customerBvn);
    ...
    // (moved from service — no Spring MVC annotations here)
    // ─────────────────────────────────────────────────────────────

    public Map<String, Object> getApprovedApplication(String customerBvn) {
        Map<String, Object> response = new HashMap<>();
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            ...
                            <p>To proceed, please click the button below to fill out the guarantor form:</p>
                            <div style="text-align: center; margin: 30px 0;">
                                <a href="%s" class="button">Fill Guarantor Form</a>
                            </div>
        ...
        return String.format("""
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <div class="warning">
                                <strong>⚠️ Action Required:</strong> Please verify guarantor information by phone call before clicking confirm.
                            </div>
                            <div class="section">
                                <h3>👤 Customer Information</h3>
                                <div class="info-row"><span class="info-label">Name:</span> %s %s</div>
                                <div class="info-row"><span class="info-label">Email:</span> %s</div>
                                <div class="info-row"><span class="info-label">BVN:</span> %s</div>
                                <div class="info-row"><span class="info-label">Plan:</span> %s</div>
                            </div>
                            <div class="section">
                                <h3>🤝 Submitted Guarantor Information</h3>
                                <div class="info-row"><span class="info-label">BVN:</span> %s</div>
                                <div class="info-row"><span class="info-label">Phone:</span> %s</div>
                                <div class="info-row"><span class="info-label">NIN:</span> %s</div>
                            </div>
                            <div class="section">
                                <h3>✅ Verified NIN Details from Mono</h3>
                                <div class="info-row"><span class="info-label">Full Name:</span> %s %s %s</div>
                                <div class="info-row"><span class="info-label">Gender:</span> %s</div>
                                <div class="info-row"><span class="info-label">DOB:</span> %s</div>
                                <div class="info-row"><span class="info-label">Phone:</span> %s</div>
                                <div class="info-row"><span class="info-label">Address:</span> %s, %s, %s, %s</div>
                            </div>
                            <div style="text-align: center; margin: 30px 0;">
                                <p><strong>After verifying via phone call, click below to confirm and send OTP:</strong></p>
                                <a href="%s" class="button">✓ Confirm Guarantor & Send OTP</a>
                            </div>
                        </div>
                    </div>
                </body>
                ...
                safe(data.getFirstName()), safe(data.getMiddleName()), safe(data.getSurname()),
                safe(data.getGender()),
                safe(data.getBirthDate()),
                safe(data.getTelephoneNo()),
                safe(data.getResidenceAddress()), safe(data.getResidenceTown()),
                safe(data.getResidenceLga()), safe(data.getResidenceState()),
                confirmUrl
        );
    }
