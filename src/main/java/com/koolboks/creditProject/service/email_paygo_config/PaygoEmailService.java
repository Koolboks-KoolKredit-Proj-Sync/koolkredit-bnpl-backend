...
public class PaygoEmailService {
    ...
    private String buildEmailContent(Map<String, Object> data) {
        try {
            ...
            html.append(buildSection("Customer Information", new String[][]{
                {"👤 Full Name", safeGet(data, "customerFirstName") + " " + safeGet(data, "customerLastName")},
                {"📧 Email", safeGet(data, "customerEmail")},
                {"📱 Phone Number", safeGet(data, "customerPhoneNumber")},
                {"🔖 Loan Reference", safeGet(data, "customerLoanRef")},
                {"⏱️ Loan Duration", safeGet(data, "customerLoanDuration") + " months"}
            }, "#000000"));

            // Agent Information - NULL SAFE
            html.append(buildSection("Agent Information", new String[][]{
                {"👨‍💼 Agent Name", safeGet(data, "agentName")},
                {"🆔 Agent ID", safeGet(data, "agentId")},
                {"📧 Email", safeGet(data, "agentEmail")},
                {"📱 Phone Number", safeGet(data, "agentNumber")}
            }, "#0a0a0a"));

            // Store & Payment Information - NULL SAFE
            html.append(buildSection("Store & Payment Details", new String[][]{
                {"🏪 Store Name", safeGet(data, "storeName")},
                {"📍 Store Location", safeGet(data, "storeLocation")},
                {"💰 Initial Installment", "₦" + safeGet(data, "initialInstalment")},
                {"📅 Payment Date", safeGet(data, "paymentDate")}
            }, "#000000"));

            // Product Information - NULL SAFE
            html.append(buildSection("Product Information", new String[][]{
                {"📦 Product Name", safeGet(data, "productName")},
                {"🏷️ Brand", safeGet(data, "productBrand")},
                {"📏 Size", safeGet(data, "productSize")}
            }, "#0a0a0a"));
            ...
            System.err.println("❌ ERROR in buildEmailContent: " + e.getMessage());
            e.printStackTrace();
            throw new RuntimeException("Failed to build email HTML", e);
        }
    }

    /**
     * Null-safe method to get data from map
     */
    ...
    private String buildButtonUrl(Map<String, Object> data) {
        try {
            ...
            url.append("customerFirstName=").append(URLEncoder.encode(safeGet(data, "customerFirstName"), "UTF-8"));
            url.append("&customerLastName=").append(URLEncoder.encode(safeGet(data, "customerLastName"), "UTF-8"));
            url.append("&customerEmail=").append(URLEncoder.encode(safeGet(data, "customerEmail"), "UTF-8"));
            url.append("&customerPhoneNumber=").append(URLEncoder.encode(safeGet(data, "customerPhoneNumber"), "UTF-8"));
            url.append("&customerLoanRef=").append(URLEncoder.encode(safeGet(data, "customerLoanRef"), "UTF-8"));
            url.append("&customerLoanDuration=").append(URLEncoder.encode(safeGet(data, "customerLoanDuration"), "UTF-8"));

            url.append("&agentName=").append(URLEncoder.encode(safeGet(data, "agentName"), "UTF-8"));
            url.append("&agentId=").append(URLEncoder.encode(safeGet(data, "agentId"), "UTF-8"));
            url.append("&agentEmail=").append(URLEncoder.encode(safeGet(data, "agentEmail"), "UTF-8"));
            url.append("&agentNumber=").append(URLEncoder.encode(safeGet(data, "agentNumber"), "UTF-8"));

            url.append("&storeName=").append(URLEncoder.encode(safeGet(data, "storeName"), "UTF-8"));
            url.append("&storeLocation=").append(URLEncoder.encode(safeGet(data, "storeLocation"), "UTF-8"));
            url.append("&initialInstalment=").append(URLEncoder.encode(safeGet(data, "initialInstalment"), "UTF-8"));
            url.append("&paymentDate=").append(URLEncoder.encode(safeGet(data, "paymentDate"), "UTF-8"));

            url.append("&productName=").append(URLEncoder.encode(safeGet(data, "productName"), "UTF-8"));
            url.append("&productBrand=").append(URLEncoder.encode(safeGet(data, "productBrand"), "UTF-8"));
            url.append("&productSize=").append(URLEncoder.encode(safeGet(data, "productSize"), "UTF-8"));
    ...
    /**
     * Build a section with data rows (NULL SAFE)
     */
    private String buildSection(String title, String[][] rows, String bgColor) {
        StringBuilder html = new StringBuilder();
        ...
        for (int i = 0; i < rows.length; i++) {
            ...
            html.append("</span>");
            html.append("<span style=\"color: #ffffff; font-size: 14px; font-weight: 600; text-align: right;\">");
            html.append(row[1]); // Already null-safe from safeGet()
            html.append("</span>");
            html.append("</div>");
        }
